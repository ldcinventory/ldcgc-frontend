...
import {
  fecthConsumables,
  fetchAddConsumable,
  fetchConsumablesLoose,
  fetchDeleteConsumable,
  fetchUpdateConsumable,
  fetchUploadConsumablesExcel,
} from "./consumablesApi"
import {
  fetchDeleteConsumableImages,
  fetchUploadConsumableImages,
} from "../../drive/driveApi"
...
export const getConsumables = createAsyncThunk<
  ...
  ConsumableParams,
  { state: RootState }
>("consumables/list", async (queryParams, thunkApi) => {
  ...
  const response = await fetchConsumablesLoose(newParams)
    ...
    .catch((error: string) => {
      throw new Error(`The server responded with an error: ${error}`)
    })

  return response
})

const updateQueryParams = createAsyncThunk<
  any,
  ConsumableParams,
  { state: RootState }
>("consumables/updateParams", async (queryParams) => {
  return queryParams
})

export const uploadConsumablesExcel = createAsyncThunk<
  any,
  FormData,
  { state: RootState }
>("consumables/excel", (formData, thunkApi) => {
  const state = thunkApi.getState()
  if (!state.users.me)
    return thunkApi.rejectWithValue("Cierra sesión e inténtalo de nuevo.")

  return fetchUploadConsumablesExcel({
    formData,
    groupId: state.users.me.group.id,
  })
    .then((res) => res.json())
    .then((json) => {
...
export const addConsumable = createAsyncThunk<
  any,
  { consumable: ConsumablePost; images: FormData },
  { state: RootState }
>("consumables/add", async ({ consumable, images }, thunkApi) => {
  let error = ""
  ...
  if (images.get("images")) {
    urlImages = await fetchUploadConsumableImages({
      images,
      consumableBarcode: consumableCreated.barcode,
    })
      .then((res) => res.json())
...
export const deleteConsumable = createAsyncThunk<
  ...
  ConsumableWithId,
  { state: RootState }
>("resources/consumables/delete", async (consumable, thunkApi) => {
    ...
    .catch((e) => {
      if (e.status === 409)
        return thunkApi.rejectWithValue(
          "No se puede eliminar el consumible porque tiene registros asociados.",
        )
...
export const updateConsumable = createAsyncThunk<
  ...
  ConsumableWithId,
  { state: RootState }
>("resources/consumables/detail/update", (consumable, thunkApi) =>
  fetchUpdateConsumable(consumable).then(() =>
    thunkApi.fulfillWithValue("Consumible actualizado con éxito."),
  ),
)

export const selectConsumableDetail = createAsyncThunk<any, string>(
  "resources/consumables/detail",
  (barcode) =>
    fecthConsumables({ consumablesParams: { barcode } })
      .then((res) => res.json())
      .then((json) => json.data),
)

export const addConsumableImages = createAsyncThunk<
  any,
  { consumableBarcode: string; images: FormData },
  { state: RootState }
>(
  "resources/consumables/images/add",
  ({ consumableBarcode, images }, thunkApi) =>
    fetchUploadConsumableImages({ consumableBarcode, images })
      .then((res) => res.json())
      .then((json) => json.data)
      .catch((e) =>
        thunkApi.rejectWithValue(`Error al añadir las imágenes: ${e.message}`),
      ),
)

export const deleteConsumableImage = createAsyncThunk<
  any,
  DriveParams,
  { state: RootState }
>("resources/consumables/images/delete", (driveParams, thunkApi) =>
  fetchDeleteConsumableImages(driveParams)
    .then((res) => res.json())
    .then((json) => driveParams.imageIds)
    .catch((e) =>
      thunkApi.rejectWithValue(`Error al eliminar la imagen: ${e.message}`),
    ),
)
...
export const consumablesSlice = createSlice({
  ...
  initialState,
  reducers: {
    selectConsumableToDelete: (
      state,
      action: PayloadAction<ConsumableWithId | null>,
    ) => {
      return { ...state, consumableToDelete: action.payload }
    },
    updateConsumableDetail: (
      state,
      action: PayloadAction<ConsumableWithId>,
    ) => {
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(
        ...
        (
          state,
          action: PayloadAction<{ data: PaginatedResponse<ConsumableWithId> }>,
        ) => {
      ...
      .addCase(
        selectConsumableDetail.fulfilled,
        (state, action: PayloadAction<ConsumableWithId>) => {
          state.consumableDetail = action.payload
        },
      )
      .addCase(
        uploadConsumablesExcel.fulfilled,
        (
          state,
          action: PayloadAction<
      ...
      .addCase(
        uploadConsumablesExcel.rejected,
        (
          state,
          action: PayloadAction<
            RejectedWithValueActionFromAsyncThunk<AnyAsyncThunk>,
      ...
      .addCase(
        deleteConsumableImage.fulfilled,
        (state, action: PayloadAction<string[]>) => {
          if (state.consumableDetail)
            state.consumableDetail.urlImages =
              state.consumableDetail.urlImages.filter(
                (i) => !action.payload.some((result) => i.includes(result)),
              )
          toast.success("Las imágenes se han eliminado correctamente.")
        },
      )
      .addCase(
        deleteConsumableImage.rejected,
        (
          state,
          action: PayloadAction<
            RejectedWithValueActionFromAsyncThunk<AnyAsyncThunk>,
...
export const { selectConsumableToDelete, updateConsumableDetail } =
