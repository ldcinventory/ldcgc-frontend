...
export const VolunteersTable = () => {
  ...
  return (
    <>
      <Toaster />    
      {/* Table mobile */}
      <ul className=" px-6 pb-2 pt-4 flex-col gap-2 bg-primary-2 dark:bg-primary-8 rounded-lg flex sm:hidden">
        <li className={`flex justify-between ${!showFilters && 'border-b pb-2 border-primary-1 dark:border-primary-9'}`}>
          <strong className="flex-1">Datos</strong>
          <strong className="flex-1">Activo</strong>
          <AppButton onClick={toggleShowFilters} className={`${showFilters && 'bg-primary-4'}`}><Bars2Icon className="h-6" /></AppButton>
        </li>
        <li className={`${!showFilters && 'hidden'} flex justify-between gap-2 border-b pb-2 border-primary-1 dark:border-primary-9`}>
        ...
        <li className="flex justify-between w-full border-t border-primary-1 dark:border-primary-9 pt-2">
          <nav>
            <AppButton onClick={(e) => handleEditVolunteersParams({ pageIndex: 0 })}><ChevronDoubleLeftIcon className="h-6" /></AppButton>
            <AppButton onClick={(e) => handleEditVolunteersParams({ pageIndex: Math.max(0, volunteersState.actualPage - 1) })}><ChevronLeftIcon className="h-6" /></AppButton>
          </nav>
          ...
          <nav>
            <AppButton onClick={(e) => handleEditVolunteersParams({ pageIndex: Math.min(volunteersState.totalPages, volunteersState.actualPage + 1) })}><ChevronRightIcon className="h-6" /></AppButton>
            <AppButton onClick={(e) => handleEditVolunteersParams({ pageIndex: volunteersState.totalPages - 1 })}><ChevronDoubleRightIcon className="h-6" /></AppButton>
          </nav>
        </li>
      </ul>
      {/* Table desktop */}
      <ul className=" px-6 pb-2 pt-4 flex-col gap-2 bg-primary-2 dark:bg-primary-8 rounded-lg hidden sm:flex">
        <li className={`flex justify-between ${!showFilters && 'border-b pb-2 border-primary-1 dark:border-primary-9'}`}>
